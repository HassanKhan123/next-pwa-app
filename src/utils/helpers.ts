...
export const groupByDate = (history: { id: number; value: string; timestamp: string }[]) => {
  const grouped: { [key: string]: { id: number; value: string }[] } = {};

  history.forEach(({ id, value, timestamp }) => {
    let responseDate;
    try {
    ...
    grouped[groupLabel].push({ id, value });
  });
